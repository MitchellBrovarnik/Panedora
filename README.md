...
*   **Waves** — Flowing, multi-layered parallax SVG waves at the bottom of the screen.
*   **Orbs** — Three large, blurred, floating spheres that drift slowly across the background.
*   **Space** — A three-layer parallax starfield that pans downward continuously.
*   **Grid** — A perspective-projected synthwave 3D floor grid that scrolls toward you.
*   **Particles** — 30 glowing dots that fall from the top of the screen like digital snow.
*   **Rings** — Four concentric circles that expand outward from the center and fade out.
*   **Reactive (Bars)** — A Canvas-based audio frequency bar visualizer that reacts to the currently playing audio in real time.
*   **Reactive (Circle)** — A Canvas-based radial equalizer ring that pulses with the audio.
*   **Reactive (Wave)** — A Canvas-based oscilloscope waveform rendered with heavy smoothing for fluid motion.
*   **Static** — No effect; only the theme's base radial gradient background is shown.
