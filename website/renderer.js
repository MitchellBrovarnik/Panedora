...
function renderSearchPage() {
    ...
    document.querySelectorAll('#search-songs .track-row').forEach((row, index) => {
        row.addEventListener('click', () => {
            if (songs[index]) {
                console.log("Mock playItem")(songs[index]);
            }
        });
    });
    ...
    document.querySelectorAll('#search-artists .card').forEach((card, index) => {
        card.addEventListener('click', () => {
            if (artists[index]) {
                console.log("Mock playItem")(artists[index]);
            }
        });
    });
    ...
    document.querySelectorAll('#search-stations .card').forEach((card, index) => {
        card.addEventListener('click', () => {
            if (stations[index]) {
                ...
                console.log("Mock playItem")({ ...s, type: 'station', id: stationSeed });
            }
        });
    });
}
...
function renderNowPlayingPage() {
    ...
    document.getElementById('np-thumbup')?.addEventListener('click', function () {
        ...
        if (isCurrentlyLiked) {
            ...
            if (token) console.log("undo feedback");
        } else {
            // Add the like
    ...
    document.querySelectorAll('.history-undo-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const result = { success: true }; console.log("undo");
            if (result && result.success) {
...
async function fetchLyrics(artist, title) {
    ...
    try {
        ...
        const result = { lyrics: "Lyrics mocked in web version." };
...
function playStation(station) {
    ...
    // Mock playItem
    updatePlayerUI({
        track: "Playing: " + station.name,
        artist: "Demo Artist",
        album: "Website Demo",
        time: 0,
        duration: 240,
        isPlaying: true,
        coverArt: station.image || "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?q=80&w=500&auto=format&fit=crop",
        trackToken: "mock-token-" + station.id
    });
}
...
const debouncedSearch = debounce(async (query) => {
    ...
    AppState.searchResults = {
        songs: [
            { title: "Search Result Song 1", artist: "Demo Artist", coverArt: "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?q=80&w=500&auto=format&fit=crop" }
        ],
        artists: [
            { name: "Search Result Artist", image: "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?q=80&w=500&auto=format&fit=crop" }
        ],
        stations: [
            { name: "Search Result Station", image: "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?q=80&w=500&auto=format&fit=crop" }
        ]
    };
...
function initEventListeners() {
    ...
    DOM.playPauseBtn.addEventListener('click', () => {
        // Mock toggle
        AppState.playerState.isPlaying = !AppState.playerState.isPlaying;
        updatePlayerUI(AppState.playerState);
    });
    // DOM.prevBtn event listener moved down
    DOM.nextBtn.addEventListener('click', () => { if (rateLimitOk('skip')) console.log("Mock next"); });
    ...
    if (titleMinBtn) titleMinBtn.addEventListener('click', () => console.log("Mock min"));
    if (titleMaxBtn) titleMaxBtn.addEventListener('click', () => console.log("Mock max"));
    if (titleCloseBtn) titleCloseBtn.addEventListener('click', () => console.log("Mock close"));
    ...
    DOM.progressBar.addEventListener('click', (e) => {
        ...
        AppState.playerState.time = percent * AppState.playerState.duration;
        DOM.progressFill.style.width = `${percent * 100}%`;
        DOM.currentTime.textContent = formatTime(AppState.playerState.time);
    });
}

// ============================================================================
// API Event Handlers (Mocked)
// ============================================================================
...
function initAPIListeners() {
    ...
    updatePlayerUI({
        ...
        trackToken: "mock-token",
        history: []
    });

    // Mock stations
    AppState.stations = [
        { id: '1', name: 'Synthwave Radio', type: 'station', image: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=256&auto=format&fit=crop' },
        { id: '2', name: 'Chill Lofi', type: 'playlist', image: 'https://images.unsplash.com/photo-1516280440502-861036214c71?q=80&w=256&auto=format&fit=crop' },
        { id: '3', name: 'Classic Rock', type: 'station', image: 'https://images.unsplash.com/photo-1498038432885-c6f3f1b912ee?q=80&w=256&auto=format&fit=crop' },
        { id: '4', name: 'Pop Hits', type: 'station', image: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=256&auto=format&fit=crop' },
        { id: '5', name: 'Jazz Classics', type: 'station', image: 'https://images.unsplash.com/photo-1511192336575-5a79af67a629?q=80&w=256&auto=format&fit=crop' },
        { id: '6', name: 'Electronic Dance', type: 'station', image: 'https://images.unsplash.com/photo-1514320291840-2e0a9bf2a9ae?q=80&w=256&auto=format&fit=crop' },
        { id: '7', name: 'Acoustic Covers', type: 'station', image: 'https://images.unsplash.com/photo-1460036521480-c16751b6ad72?q=80&w=256&auto=format&fit=crop' },
        { id: '8', name: 'Indie Mix', type: 'playlist', image: 'https://images.unsplash.com/photo-1459749411175-04bf5292ceea?q=80&w=256&auto=format&fit=crop' }
    ];
    ...
    setInterval(() => {
        if (AppState.playerState.isPlaying) {
            AppState.playerState.time += 1;
            if (AppState.playerState.time >= AppState.playerState.duration) {
                AppState.playerState.time = 0;
                AppState.playerState.isPlaying = false;
            }
            updatePlayerUI({ time: AppState.playerState.time });
        }
    }, 1000);
}
